...
export async function generateInvoice(order: any): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      cell(doc, billToLeft + sW, addrY, cW, addrRowH, `Code  ${COMPANY.stateCode}`, { fontSize: 6.5 })
      ...
      cell(doc, shipToLeft + sW, addrY, cW, addrRowH, `Code  ${COMPANY.stateCode}`, { fontSize: 6.5 })
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        cell(doc, colX, rowY, cols[6], rH, "-", { fontSize: 6.5, align: "center" }); colX += cols[6]
