...
async function createCalComBooking(params: {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const match = Array.isArray(types)
        ? types.find((t: any) => t.slug === params.eventSlug)
        : null;
      if (match) eventTypeId = match.id;
    }
  } catch (e: any) {
    console.warn(
      "[Razorpay Webhook] Cal.com event-types lookup failed:",
      e.message,
    );
  }
  ...
  if (!res.ok) {
    console.error(
      "[Razorpay Webhook] Cal.com booking error:",
      res.status,
      json,
    );
    return {};
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  if (!email || !firstName) {
    console.warn(
      `[Razorpay Webhook] Payment ${razorpayPaymentId} missing notes — skipping`,
    );
    return res
      .status(200)
      .json({ message: "No booking notes found — skipping" });
  }

  console.log(
    `[Razorpay Webhook] payment.captured for ${email} | ${razorpayPaymentId}`,
  );
  ...
  try {
    ...
    if (existingOrders.length > 0) {
      console.log(
        `[Razorpay Webhook] Order already exists for ${razorpayPaymentId} — skipping`,
      );
    ...
    if (!region) {
      console.error(
        `[Razorpay Webhook] No region found — cannot create order for ${email} | ${razorpayPaymentId}`,
      );
    ...
    try {
      ...
      console.warn(
        "[Razorpay Webhook] Could not resolve sales channel:",
        e.message,
      );
    }
    ...
    try {
      ...
      console.warn(
        "[Razorpay Webhook] Could not resolve/create customer:",
        e.message,
      );
    }
    ...
    try {
      ...
      console.error(
        "[Razorpay Webhook] Could not register payment:",
    ...
    if (hasSession && !isPackage && slotIsoStart && eventSlug) {
      ...
      if (calApiKey && calUsername) {
        try {
          console.log(
            `[Razorpay Webhook] Creating Cal.com booking for ${email} | ${eventSlug} | ${slotIsoStart}`,
          );
          ...
          if (calResult.uid || calResult.meetUrl) {
            // If Cal.com returned the booking UID but the video room wasn't ready yet,
            // wait briefly and retry fetching the meetUrl before saving
            if (calResult.uid && !calResult.meetUrl) {
              console.log(
                `[Razorpay Webhook] Cal.com returned UID but no meetUrl — retrying in 3s...`,
              );
              await new Promise((r) => setTimeout(r, 3000));
              try {
                const retryAbort = new AbortController();
                const retryTimeout = setTimeout(() => retryAbort.abort(), CAL_TIMEOUT_MS);
                const retryRes = await fetch(
                  `${CAL_API_BASE}/bookings/${calResult.uid}`,
                  {
                    headers: {
                ...
                if (retryRes.ok) {
                  ...
                  const booking = retryJson.data || retryJson;
                  const retryMeetUrl =
                    booking?.meetingUrl ||
                    booking?.metadata?.videoCallUrl ||
                    booking?.references?.find((r: any) => r.meetingUrl)?.meetingUrl;
                  if (retryMeetUrl && typeof retryMeetUrl === "string" && retryMeetUrl.startsWith("http")) {
                    calResult.meetUrl = retryMeetUrl;
                    console.log(
                ...
                console.warn(
                  `[Razorpay Webhook] meetUrl retry failed (non-blocking):`,
                  retryErr.message,
                );
              }
            }
          ...
          console.error(
            "[Razorpay Webhook] Cal.com booking failed (non-blocking):",
            calErr.message,
          );
        }
      } else {
        console.warn(
          "[Razorpay Webhook] CAL_API_KEY or CAL_USERNAME not set — skipping Cal.com booking",
        );
      }
    }
    ...
    if (variantId) {
      try {
        ...
        console.error(
          "[Razorpay Webhook] Google Drive sharing failed (non-blocking):",
          driveErr.message,
        );
      }
    }
    ...
    try {
      ...
      console.log(
        `[Razorpay Webhook] Emitted order.placed for order ${order.id}`,
      );
    } catch (eventErr: any) {
      console.error(
        "[Razorpay Webhook] Failed to emit order.placed:",
        eventErr.message,
      );
    }
    ...
    return res
      .status(200)
      .json({ message: "Processed with error", error: error.message });
  }
}
...
async function handleRefund(req: MedusaRequest, res: MedusaResponse) {
  try {
    ...
    if (!order) {
      console.warn(
        `[Razorpay Webhook] No order found for payment ${paymentId}`,
      );
      return res.status(200).json({ received: true });
    }
    ...
    console.log(
      `[Razorpay Webhook] Refund handled for Order #${order.display_id}`,
    );
    return res.status(200).json({ received: true });
  } catch (error: any) {
