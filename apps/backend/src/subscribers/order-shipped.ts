...
export default async function orderShippedHandler({
  ...
  try {
    ...
    if (!orderId) {
      console.error(`[Order Shipped] No order linked to fulfillment #${data.id}.`)
      return
    }
    ...
    if (!order) {
      console.error(`[Order Shipped] Order #${orderId} not found.`)
      return
    }

    // Skip session/digital orders — nothing physically ships
    if (order.metadata?.is_session === true || order.metadata?.is_session === "true") {
      return
    }

    if (!notificationService) {
      console.warn("[Order Shipped] Notification service not available.")
      return
    }
    ...
    // data.id is the fulfillment ID — find it directly
    const fulfillment =
      (order.fulfillments || []).find((f: any) => f.id === data.id) ||
      (order.fulfillments || []).at(-1)

    const trackingLabel = fulfillment?.labels?.[0]
    const trackingNumber = trackingLabel?.tracking_number || ""
    const trackingUrl = trackingLabel?.tracking_url || ""
    ...
    const trackingHtml = trackingNumber
      ? `
        <div style="background: #F9F7F9; border-left: 4px solid #C5A059; border-radius: 4px; padding: 20px; margin: 25px 0;">
          <p style="margin: 0 0 6px; font-weight: 700; font-size: 14px; color: #2C1E36;">Tracking Information</p>
          <p style="margin: 0 0 4px; font-size: 13px; color: #665D6B;">Tracking Number: <strong style="color: #2C1E36;">${trackingNumber}</strong></p>
          ${
            trackingUrl
              ? `<a href="${trackingUrl}" style="display: inline-block; margin-top: 12px; background: #2C1E36; color: #C5A059; padding: 10px 22px; border-radius: 8px; text-decoration: none; font-size: 13px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase;">Track Your Order</a>`
              : ""
          }
        </div>`
      : `
        <div style="background: #F9F7F9; border-left: 4px solid #C5A059; border-radius: 4px; padding: 20px; margin: 25px 0;">
          <p style="margin: 0; font-size: 13px; color: #665D6B;">Tracking details will be shared as soon as your package is picked up by the courier.</p>
        </div>`
    ...
    const htmlBody = `
      <div style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; background-color: #FBFAF8; padding: 40px 30px; color: #110E17; max-width: 620px; margin: 0 auto; border: 1px solid #E1DFE3; border-radius: 24px;">
        ...
        <!-- TITLE -->
        <h2 style="font-size: 26px; font-weight: 800; color: #2C1E36; margin: 0 0 10px;">Your order is on its way! ✨</h2>
        <p style="font-size: 15px; line-height: 1.5; color: #665D6B; margin: 0 0 25px;">
          Hi ${firstName}, your order <a href="${storefrontUrl}/order/lookup?display_id=${order.display_id}&email=${order.email}" style="color: #7C3AED; text-decoration: none; font-weight: 600;">#${order.display_id}</a> has been dispatched and is on its way to you.
        </p>
        ...
        <div style="margin-bottom: 25px;">
          <h3 style="font-size: 16px; font-weight: 700; color: #2C1E36; margin: 0 0 12px;">Items shipped</h3>
        ...
        <div style="border-top: 1px solid #E1DFE3; padding-top: 20px; margin-bottom: 25px;">
          <p style="font-size: 14px; font-weight: 700; color: #2C1E36; margin: 0 0 10px;">Shipping to</p>
    ...
    await (notificationService as any).createNotifications([
      {
        ...
        data: {
          subject: `Your order #${order.display_id} has been shipped – The Blissful Soul`,
          html_body: htmlBody,
        },
      },
    ])

    console.log(`[Order Shipped] Shipment notification sent for Order #${order.display_id} to ${order.email}`)
  } catch (error: any) {
    console.error(`[Order Shipped] Error for Order #${data.id}:`, error.message || error)
  }
}
