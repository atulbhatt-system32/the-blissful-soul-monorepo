        ...
        <div className="flex flex-col items-center justify-center pt-24 pb-20">
          <div className="w-full max-w-3xl relative group">
            <input 
              ...
              className="w-full text-center bg-transparent border-b-2 border-[#C5A059]/20 py-6 text-4xl md:text-6xl font-serif text-[#2C1E36] outline-none focus:border-[#C5A059] transition-all uppercase tracking-normal placeholder:text-[#2C1E36]/10 px-12"
              value={query}
          ...
          {!hasSearched ? (
             <div className="mt-12 text-center animate-in fade-in slide-in-from-bottom-2 duration-700">
                ...
                <div className="flex flex-wrap justify-center gap-3 text-[10px] uppercase tracking-[0.2em] font-bold text-[#C5A059]/60">
                   <span>Amethyst</span> • <span>Rose Quartz</span> • <span>Citrine</span> • <span>Sessions</span>
                </div>
             </div>
          ) : (
            <button 
        ...
        {hasSearched && (
          <div className="w-full flex-1 pt-10 border-t border-[#2C1E36]/5 animate-in fade-in duration-500">
            ...
            <div className="mb-16 text-center py-6">
                <span className="text-[10px] text-[#C5A059] font-bold uppercase tracking-[0.4em] mb-4 block">Discovery</span>
                <h2 className="text-3xl md:text-5xl font-serif text-[#2C1E36] uppercase tracking-tight">
                   Results for <span className="italic font-normal opacity-60">"{query}"</span>
                </h2>
            ...
            {isLoading ? (
               <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                  {Array.from({ length: 4 }).map((_, i) => (
                      <div key={i} className="aspect-[4/5] bg-[#2C1E36]/5 rounded-3xl animate-pulse" />
                  ))}
               </div>
            ) : (
              <div className="pb-24">
                {products.length > 0 ? (
                  <ul className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-16">
                    {products.map((p) => (
                      <li key={p.id} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                {products.length > 0 && (
                  <div className="mt-32 text-center pb-20 border-t border-[#2C1E36]/5 pt-20">
                     ...
                     >
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#2C1E36]/60 group-hover:text-[#2C1E36]">Close Search</span>
                     </button>
                  </div>
                )}
              </div>
